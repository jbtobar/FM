...
contract Temp {
  ...
  function SetMappingLength (uint ind1, uint length) public {
    map[ind1].length = length;
  }

  function SetMappingElLength (uint ind1, uint ind2, uint length) public {
    map[ind1][ind2].arr.length = length;
  }

  function SetMapping (uint ind1, uint ind2, uint a, uint b, uint ind3, uint val3) public {
    map[ind1][ind2].a = a;
    map[ind1][ind2].b = b;
